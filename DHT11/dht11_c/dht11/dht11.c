...
static int dht11_decode(void) {
    int i, bitM = DHT11_DATA_BIT_LOW * 3 / 2 + 50, bitN = DHT11_DATA_BIT_HIGH*2/3 + 50, wtime=0;
...
{
    int result;
    result = request_irq(gpio_to_irq(gpio_pin), (irq_handler_t) irq_handler, IRQF_TRIGGER_FALLING, DHT11_DRIVER_NAME, NULL);
...
start_read:
    ...
    ret = wait_for_completion_killable_timeout(&gpio_completion, TOTAL_INT_BLOCK);
    clear_interrupts();
